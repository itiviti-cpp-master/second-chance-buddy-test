...
using TestedTypes = ::testing::Types<Params<1, 1>, Params<1, 24>, Params<3, 1>, Params<7, 4>, Params<7, 41>, Params<10, 20>, Params<256, 1>, Params<256, 256>>;
...
TYPED_TEST(AllocatorTest, dummy_fragmentation)
{
    ...
    if (this->dummy_cost < this->complex_cost) {
        ...
        for (std::size_t i = 0; i < freed; ++i) {
            EXPECT_NO_THROW(c_ptrs.push_back(this->create_complex(n, x, d))) << "Failed to create " << i << " out of " << freed << " complex element";
            ++n;
        }
    }
...
TYPED_TEST(AllocatorTest, dummy_defragmentation)
{
    ...
    const std::size_t dummy_leftover = (this->dummy_cost * free_batch - complex_in_batch * this->complex_cost) / this->dummy_cost;
    std::size_t freed = 0;
